...
a linear program $\max\{c^t x \mid Ax\leq b\}$ is feasible and unbounded
...
%convex combination of vectors in $P \setminus\{x'\}$. 
%``$\Leftarrow$:'' 
The vector $x'$ is a vertex of $P$ is and only if it is a
...
Moreover, complementary slackness  (Theorem~\ref{theorem::ComplementaySlackness}) adapted to this form asserts that whenever $x$ and $y$ are primal and dual feasible, and $A^t_i y=0$ whenever $x_i>0$, then both of them are optimal. Since every $i$ for which $x_i>0$ is contained in $B$, and $A_B^t y= c_B$, it follows that $x$ and $y$ satisfy complementary slackness.
\endproof
...
If $c_B^t A_B^{-1}$ is a feasible dual solution,
...
\end{array}
\end{equation}
Complementary slackness (Theorem~\ref{theorem::ComplementaySlackness}) in this form asserts that $x\in\Rp^n$ and $(y,s)\in\R^m\times \Rp^n$ if and only if $x^t s=0$. Given that both vectors are nonnegative, this is equivalent to $x_i=0$ or $s_i=0$ for every $i=1,2,\ldots,n$. Moreover, for primal and dual solutions $x\in\Rp^n$ and $(y,s)\in\R^m\times \Rp^n$,  $x^t s$ is the optimality gap between these solutions:
...
\end{equation}
The latter term is called the \emph{logarithmic barrier function}. It enforces in particular that $x_i>0$ for all $i=1,2,\ldots,n$. Intuitively, if $\lambda>0$ but is very small, then the solution must be feasible and nearly optimal.
...
and 
\[
    \|\Delta  x\circ \Delta  y\|\le \frac{\|z\circ v\|^2}{2}\le \frac{\|v\|^2}{2\min_{i\in[n]}{x_is_i}}\, .
\]
%If $(x,y,s)\in\N(\beta)$, then 
\end{lemma}
\prove
...
Let $W=\ker(AD)$ and $W^\perp=\mathrm{im}((AD)^t)$. Then, the first two sets of equations are equivalent to $\Delta \hat x\in W$ and $\Delta \hat s\in W^\perp$ (the role of $y$ is to represent $\Delta \hat s$ is $W^\perp$).
 ...
 Proposition~\ref{prop:orth-decomp}. Uniqueness of $\Delta\hat y$ follows because of the assumption $\mathrm{rank}(A)=m$. From Lemma~\ref{lem:orth-decomp-norm}(b), we get that
 \[
       \|\Delta  \hat x\circ \Delta \hat y\|\le \frac{1}{2}\|z\circ v\|^2\, .
 \]
Next, note that
 \[
 ...
 \]
 Using also  
 Lemma~\ref{lem:orth-decomp-norm}(a) for $z$ and $v$,
...
\[
    \Delta x^t \Delta s= \Delta x^t(-A^t\Delta y)=(A\Delta x)^t \Delta y=0\, ,
\]
or in other words, $\Delta x\in \ker(A)$ and $\Delta x\in\mathrm{im}(A)$ live in orthogonal subspaces.
...
$y^t A = c$ and $y \geq 0$. By Farkas' Lemma (Theorem~\ref{theorem::farkas2}),
...
possible due to Corollary~\ref{corollary::computation_of_integer_solutions}.
\endproof
